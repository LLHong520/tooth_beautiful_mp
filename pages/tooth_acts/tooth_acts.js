import pageCommonFns from "../../scripts/pageCommonFns.js";
import activistService from "../../services/activistService.js";
//index.js
//获取应用实例
var app = getApp();
var pageConfigs={
  ...pageCommonFns,
  data: {
    hide_loading: true,
    article: {
      list: [],
      hasMore: true,
      load_more: false,
      load_text: "加载中...",
      noMoreArticle: false,
      pager: {
        pageSize: 10,
        pageIndex: 0,
      }
    }
  },
  get_article() {
    if (!this.data.article.hasMore) return;
    var This = this;
    This.data.article.pager.pageIndex = This.data.article.pager.pageIndex + 1;
    if (This.data.article.pager.pageIndex == 1) {
      this.setData({
        hide_loading: false
      });
    } else {
      this.setData({
        "article.load_more": true
      });
    }
    activistService.activists({
      size: This.data.article.pager.pageSize,
      page: This.data.article.pager.pageIndex
    })
    .then(res => {
      wx.hideNavigationBarLoading();
      if (This.data.article.pager.pageIndex == 1) {
        wx.stopPullDownRefresh();
        This.setData({
          hide_loading: true
        });
      } else {
        This.setData({
          "article.load_more": false
        });
      }
      if (res.code == 0 && res.data) {
        This.setData({
          "article.list": This.data.article.list.concat(res.data),
        });
        //确定没有更多时
        let total = Math.ceil(res.total / This.data.article.pager.pageSize);
        if (This.data.article.pager.pageIndex + 1 > total) {
          this.setData({
            "article.load_more": false,
            "article.noMoreArticle": true,
            "article.load_text": "沒有更多了",
          });
        } else {
          this.setData({
            "article.load_more": true,
            hide_loading: true
          });
        }
      }
    })
    .catch(res => {
      This.setData({
        "article.load_more": false,
        hide_loading: true
      });
    });
  },
  //事件处理函数
  _to_top_fn: function (event) {
    if (wx.pageScrollTo) {
      wx.pageScrollTo({
        scrollTop: 0
      });
    } else {
      if (app.globalData.sysInfo.isiOS) {
        wx.showModal({
          title: "提示",
          content: "当前微信版本过低，，请升级到最新微信版本后重试或点击状态栏返回页面顶部。"
        });
      } else {
        wx.showModal({
          title: "提示",
          content: "当前微信版本过低，，请升级到最新微信版本后重试或双击导航栏返回页面顶部。"
        });
      }
    }
  },
  onPageScroll(pos) {
    if (pos.scrollTop > 600) {
      this.setData({
        "_to_top.isShow": true
      });
    } else {
      this.setData({
        "_to_top.isShow": false
      });
    }
  },
  onReachBottom: function () {
    //页面滚动到底部回调函数
    if (!this.data.article.noMoreArticle) {
      this.get_article();
    }
  },
  onLoad: function () {},
  onShow: function () {
    this._initPage();
  },
  _initPage() {
    wx.showNavigationBarLoading();
    this.setData({
      "article.list": [],
      "article.pager.pageIndex": 0,
      "article.hasMore": true,
      "article.load_more": false,
      "article.noMoreArticle": false,
    });
    this.get_article();
  },
  onPullDownRefresh: function () {
    this._initPage();
  }
};
Page(pageConfigs);
